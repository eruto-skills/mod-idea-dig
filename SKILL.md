...
name: mod-idea-dig
description: >
  Minecraft サーバー運営の次回ワールド向け MOD/datapack 候補を発掘する半自動スキル。
...
MOD 選定ディレクトリでの候補発掘を半自動で回すスキル。
**機械的な発掘・分類・下書きはこのスキルが、基準の定義と最終採否は人間が持つ。**

対象ディレクトリ: 既定は利用側プロジェクトの `mod-selection/`。
別の場所に置いているなら、呼び出し時に指定するか、そのプロジェクトの `CLAUDE.md` で定める。
このスキルは**パスを固定で持たない** — 選定ディレクトリの中身（下記の入力ファイル群）だけを前提にする。
